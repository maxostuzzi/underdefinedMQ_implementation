...
class Hashimoto(MQAlgorithm):
    ...
    def _compute_time_complexity(self, parameters: dict):
        ...
        if ((a * (m - k) - (a-1)**2 + k) <= n) and (a*(m - k) - (a - 1)^2 + k):
            ...
            com2 = k * log2(q)/2 + E_2.time_complexity()
            com3 = k * log2(q)/2 + E_3.time_complexity(k=0, variant=MQ_LAS_VEGAS)
            return log2(2**com1 + 2**com2 + 2**com3)
            # com1 = log2(m - a - k + 1) + E_1.time_complexity()
